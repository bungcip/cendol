...
## 2025-11-18 - Reusable FunctionState in MirGen to Avoid Heap Allocations
**Learning:** During AST to MIR generation (`MirGen`), `FunctionState` contains four `FxHashMap` instances (`local_map`, `vla_map`, `label_map`, `switch_case_map`) and a nested `Vec<Vec<CleanupAction>>`. Previously, a new `FunctionState` was instantiated on the heap for every function compiled in `visit_function` and dropped when exiting the function. On large translation units with thousands of functions, this triggered 5,000+ heap allocations and deallocations. Adding `reusable_func_state: Option<FunctionState>` on `MirGen` and implementing `FunctionState::reset` allows all 4 `FxHashMap`s and `Vec`s to call `.clear()` and retain their capacity across functions, completely eliminating heap allocation churn during MIR function lowering.
**Action:** When visitor or lowering structs instantiate function-local state containing maps or collections, retain and reuse the state struct across functions with a `.reset()` / `.clear()` method to preserve allocated memory capacities across the entire compilation unit.
