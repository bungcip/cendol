...
## 2026-08-20 - Fast-path for Standard Single-Quote Char Literals in PPLexer
**Learning:** During preprocessor lexing, character literals are overwhelmingly standard non-prefixed single quotes (`'a'`, `'\n'`). `lex_char_literal` previously executed `prefix.ends_with(b"'")` and `prefix.len()` on every character literal to compute quote bounds. Fast-pathing `prefix == [b'\'']` to directly assign `(ends_with_delimiter = true, content_start = 1)` avoids slice queries and arithmetic calculations on the hot lexing path.
**Action:** Fast-path single-character delimiters/prefixes in literal lexing helpers to avoid redundant slice inspection overhead for common non-prefixed literals.
