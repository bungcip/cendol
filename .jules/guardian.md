...
2025-02-17 - [SizeOf Incomplete Type Tests]
Learning: The sizeof operator is strictly prohibited from being applied to incomplete types (like `void` or incomplete `struct S;`), as its primary job is computing memory requirements which are impossible for unknown layouts. We had the SemanticError::SizeOfIncompleteType and logic in place to emit it, but lacked a test guaranteeing it works as expected from a C perspective.
Action: Added `test_sizeof_incomplete_type_prohibited` to `src/tests/guardian_sizeof_alignof.rs` to explicitly guard this compiler invariant.
