...
2024-05-31 - [Invalid Restrict Qualifier]
Learning: The `restrict` qualifier is strictly meant for pointer types in C11. Using it on an integer, an array, or a function declaration violates language semantics. The compiler correctly enforced this via `SemanticError::InvalidRestrict`, but a dedicated diagnostic test ensuring its consistent interception across variables, arrays, and functions was absent.
Action: Add `guardian_invalid_restrict.rs` to explicitly validate that `restrict` on non-pointer types is correctly rejected during `CompilePhase::SemanticLowering` and that the diagnostic span correctly identifies the declaration.
