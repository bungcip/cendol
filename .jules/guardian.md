...
2026-03-26 - [_Alignas on Variably Modified Types]

Learning: C11 6.7.5p2 explicitly prohibits alignment specifiers on objects with variably modified types (e.g., Variable Length Arrays or pointers to VLAs). This constraint is distinct from the general prohibition of `_Alignas` on functions or `register` objects. Enforcing this in `visit_variable_decl` and `visit_struct_members` ensures that invalid alignment requirements don't reach the backend where they might cause miscompilation or crashes, especially for types whose size is only known at runtime.
Action: Always verify that type-based constraints like `_Alignas` are validated against the fully resolved type, including variably-modified status, in all declaration contexts (variables, members, etc.).
