...
## 2026-07-04 - [VLA Scope Jump Diagnostic Spans]
Learning: When writing tests for `SemanticError::JumpIntoScopeVLA` (C11 §6.8.6.1p1 and §6.8.4.2p4 constraints), the primary diagnostic span points to the instruction jumping into the scope (i.e. the `switch` condition expression or the `goto` target), not the VLA declaration itself. The VLA declaration and the jump target label are emitted as secondary `Note` diagnostics. The tests failed initially because they attempted to assert the diagnostic at the line of the VLA declaration instead of the `goto`/`switch` statement.
Action: Always double-check the codebase's error reporting logic (e.g., `report_error_with_notes` in `src/semantic/analyzer.rs`) for complex flow errors to determine exactly which node receives the primary `Error` diagnostic before asserting span lines.
