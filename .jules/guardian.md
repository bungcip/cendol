...
Learning: C11 6.7.4p1 requires that function specifiers ('inline' and '_Noreturn') only be used in the declaration of an identifier that is a function. This means they must be rejected for typedefs, struct/union members, and tag declarations (forward decls or definitions), even if the underlying type is a function pointer.
Action: Centralize function specifier validation in semantic lowering to ensure all non-function declaration paths (variables, typedefs, members, tags) correctly enforce these constraints.
2024-05-24 - [Array of Functions vs Function Returning Function]
...
2026-08-12 - [Conflicting Storage Classes constraints]

Learning: C11 (and C23 for `constexpr`) strictly enforces that at most one storage-class specifier may be given in the declaration specifiers in a declaration, except that `_Thread_local` may appear with `static` or `extern`. Without this enforcement, developers might declare things like `static extern int x;` or `typedef static int y;`, or duplicate the same storage class like `auto auto int x;`. These constructs break language constraints and could cause ambiguous behavior or codegen failures. The compiler correctly enforced these during semantic lowering via `SemanticError::ConflictingStorageClasses`, but lacked a dedicated integration test.
Action: Add `guardian_conflicting_storage_classes.rs` to validate this invariant, ensuring the diagnostic triggers at `CompilePhase::SemanticLowering` with the correct error message and specific line/column spans for multiple cases, including traditional combinations, typedef combinations, `_Thread_local`, duplicate storage class, and `constexpr`.
