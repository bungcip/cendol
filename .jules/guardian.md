...
Learning: C11 6.7.5p2 explicitly prohibits alignment specifiers (_Alignas) on objects with variably modified types. This includes not only VLAs but also pointers to VLAs. Interestingly, some existing "happy path" tests in the compiler were actually violating this constraint to test large alignments, which highlights how easily this rule can be overlooked.
Action: Enforce _Alignas constraints during semantic lowering for all object declarations, and ensure that both direct VLAs and derived variably modified types are covered.
