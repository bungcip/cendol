...
fn gen_arguments_method(formats: &[Rc<InstructionFormat>], fmt: &mut Formatter, is_mut: bool) {
    ...
    fmt.add_block(
        &format!("pub fn {method}<'a>(&'a {mut_}self, pool: &'a {mut_}ir::ValueListPool) -> &'a {mut_}[Value]"),
        |fmt| {
...
fn gen_type_constraints(all_inst: &AllInstructions, fmt: &mut Formatter) {
    ...
    fmt.indent(|fmt| {
        for inst in all_inst.iter() {
            ...
            fmt.comment(format!(
                "{}: fixed_results={}, use_typevar_operand={}, requires_typevar_operand={}, fixed_values={}",
                inst.camel_name, fixed_results, use_typevar_operand, requires_typevar_operand, fixed_values
            ));
            fmt.comment(format!(
                "Constraints=[{}]",
                constraints
            ...
            if let Some(poly) = &inst.polymorphic_info {
                fmt.comment(format!(
                    "Polymorphic over {}",
                    typeset_to_string(poly.ctrl_typevar.get_raw_typeset())
                ));
            }
