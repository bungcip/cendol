...
use crate::machinst::CallRetList;
...
#[derive(Clone, Debug)]
pub struct InlineAsmInstData {
    pub asm_id: crate::ir::InlineAsmId,
    pub template: String,
    pub uses: CallArgList,
    pub defs: CallRetList,
}
pub type BoxInlineAsmData = Box<InlineAsmInstData>;
...
impl PrettyPrint for Inst {
    fn pretty_print(&self, _size: u8) -> String {
        ...
        match self {
            ...
            Inst::InlineAsm { data } => {
                let op = ljustify("inline_asm".to_string());
                format!("{op} {:?}", data.asm_id)
            }
...
fn x64_get_operands(inst: &mut Inst, collector: &mut impl OperandVisitor) {
    ...
    match inst {
        ...
        Inst::InlineAsm { data } => {
            for CallArgPair { vreg, preg } in &mut data.uses {
                collector.reg_fixed_use(vreg, *preg);
            }
            for CallRetPair { vreg, location } in &mut data.defs {
                match location {
