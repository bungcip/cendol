...
impl LowerBackend for X64Backend {
    ...
    fn lower(&self, ctx: &mut Lower<Inst>, ir_inst: IRInst) -> Option<InstOutput> {
        if ctx.dfg().insts[ir_inst].opcode() == crate::ir::Opcode::InlineAsm {
            use crate::isa::x64::inst::{InlineAsmInstData, Inst};
            use alloc::boxed::Box;
            use smallvec::smallvec;

            let asm_id = match ctx.dfg().insts[ir_inst] {
                crate::ir::InstructionData::InlineAsm { asm_id, .. } => asm_id,
                _ => unreachable!(),
            };

            let template = ctx.dfg().inline_asms.get(asm_id).unwrap().template.clone();

            // Note: Cranelift x86_64 uses smallvec for arguments and returns.
            // For now, we will just create an empty list, but in reality we would populate
            // them based on the InlineAsmData operand lists.
            let uses = smallvec![];
            let defs = smallvec![];

            ctx.emit(Inst::InlineAsm {
                data: Box::new(InlineAsmInstData {
                    asm_id,
                    template,
                    uses,
                    defs,
