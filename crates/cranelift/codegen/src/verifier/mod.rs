        ...
        match dfg.value_def(v) {
            ...
            ValueDef::Param(block, _) => {
                ...
                let user_block = self
                    ...
                    .layout
                    .inst_block(loc_inst)
                    .expect("Expected instruction to be in a block as we're traversing code already in layout");
    ...
    fn domtree_integrity(
            ...
            .enumerate()
        {
            if test_block != true_block {
                return errors.fatal((
                    test_block,
                    format!("invalid domtree, postorder block number {index} should be {true_block}, got {test_block}"),
                ));
            }
        }
        Ok(())
            ...
            if let Some(expected_type) = expected_type {
                if result_type != expected_type {
                    errors.report((
                        inst,
                        self.context(inst),
                        format!("expected result {i} ({result}) to have type {expected_type}, found {result_type}"),
                    ));
                }
            } else {
                ...
                ResolvedConstraint::Free(type_set) => {
                    if !type_set.contains(arg_type) {
                        errors.report((
                            inst,
                            self.context(inst),
                            format!("arg {i} ({arg}) with type {arg_type} failed to satisfy type set {type_set:?}"),
                        ));
                    }
                }
            }
