...
The Semantic Lowering phase is the "First Stage" of semantic analysis. It is responsible for bridging the gap between the purely syntactic `PAst` produced by the parser and the enriched, type-resolved `Ast` used by the analyzer and codegen.
...
4. **AST Transformation**: Convert generic `PNode`s into specialized semantic `NodeKind` variants (e.g., `VarDecl`, `FunctionDecl`).
...
- Maps between `PNodeRef` and resulting `NodeRef`s.
...
C's "declaration mimics usage" syntax (e.g., `int (*f[5])(void)`) is handled by recursively applying declarator layers to a base type. The lowering phase traverses the `PDeclarator` chain to build the final `QualType`.
