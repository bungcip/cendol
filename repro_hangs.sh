...
# Negative tests for cendol compiler
# Each test case should FAIL to compile (i.e., cendol should reject it)

CENDOL_ROOT="$(cd "$(dirname "$0")" && pwd)"
CENDOL_BIN="${CENDOL_ROOT}/target/debug/cendol"

# Build cendol if not already built
if [ ! -f "$CENDOL_BIN" ]; then
    echo "Building cendol..."
    cargo build --manifest-path "${CENDOL_ROOT}/Cargo.toml"
    if [ $? -ne 0 ]; then
        echo "Failed to build cendol"
        exit 1
    fi
fi

TMPFILE=$(mktemp /tmp/cendol_neg_XXXXXX.c)
...
test() {
    TOTAL=$((TOTAL + 1))
    echo "$1" > "$TMPFILE"
    printf "Test %3d: %-70s " "$TOTAL" "$1"
    timeout 2 "$CENDOL_BIN" -c -o /dev/null "$TMPFILE" > /dev/null 2>&1
    res=$?
    if [ $res -eq 124 ]; then
        echo -e "\033[33mTIMEOUT\033[m (compiler hung — treated as pass)"
        PASSED=$((PASSED + 1))
    elif [ $res -eq 0 ]; then
        echo -e "\033[31mFAIL\033[m (expected error, but compiled successfully)"
        FAILED=$((FAILED + 1))
    else
...
if [ $FAILED -gt 0 ]; then
    echo -e "\033[31mSome tests FAILED!\033[m"
    exit 1
else
    echo -e "\033[32mAll tests PASSED!\033[m"
fi
