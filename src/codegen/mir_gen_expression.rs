    ...
    fn visit_literal(&mut self, node_kind: &NodeKind, ty: QualType) -> Option<Operand> {
        ...
        match node_kind {
            NodeKind::Literal(literal) => match literal {
                ...
                Literal::True => Some(Operand::Constant(self.create_constant(mir_ty, ConstValueKind::Bool(true)))),
                Literal::False => Some(Operand::Constant(self.create_constant(mir_ty, ConstValueKind::Bool(false)))),
            },
            _ => None,
        }
    }
