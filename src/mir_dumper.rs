...
pub struct MirDumper<'a> {
    module: &'a MirModule,
    functions: &'a HashMap<MirFunctionId, MirFunction>,
    blocks: &'a HashMap<MirBlockId, MirBlock>,
    locals: &'a HashMap<LocalId, Local>,
    globals: &'a HashMap<GlobalId, Global>,
    types: &'a HashMap<TypeId, MirType>,
    constants: &'a HashMap<ConstValueId, ConstValue>,
    statements: &'a HashMap<MirStmtId, MirStmt>,
...
impl<'a> MirDumper<'a> {
    ...
    pub fn generate_mir_dump(&self) -> Result<String, std::fmt::Error> {
        ...
        if self.config.include_header {
            writeln!(output, "; MIR Dump for Module {}", self.module.id.get())?;
        ...
        if !self.module.globals.is_empty() {
            for &global_id in &self.module.globals {
                if let Some(global) = self.globals.get(&global_id) {
    ...
    /// Generate MIR dump as string (convenience method for tests)
    pub fn generate_mir_dump_string(&self) -> Result<String, std::fmt::Error> {
        self.generate_mir_dump()
    }

    /// Generate MIR dump for a single function (convenience method for tests)
    pub fn generate_function_mir_dump(&self, func_id: MirFunctionId) -> Result<String, std::fmt::Error> {
        let mut output = String::new();
    ...
    fn dump_function(&self, output: &mut String, func: &MirFunction) -> Result<(), std::fmt::Error> {
        ...
        for (i, &param_id) in func.params.iter().enumerate() {
            ...
            if let Some(param) = self.locals.get(&param_id) {
    ...
    fn dump_block(&self, output: &mut String, block_id: MirBlockId, block: &MirBlock) -> Result<(), std::fmt::Error> {
        ...
        for &stmt_id in &block.statements {
            if let Some(stmt) = self.statements.get(&stmt_id) {
                write!(output, "    ")?;
    ...
    fn local_to_string(&self, local_id: LocalId) -> String {
        let local = self.locals.get(&local_id);
