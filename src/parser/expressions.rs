...
fn parse_prefix(parser: &mut Parser) -> Result<ParsedNodeRef, ParseError> {
    ...
    match token.kind {
        ...
        TokenKind::True => {
            parser.advance();
            Ok(parser.push_node(ParsedNodeKind::Literal(Literal::Bool(true)), token.span))
        }
        TokenKind::False => {
            parser.advance();
            Ok(parser.push_node(ParsedNodeKind::Literal(Literal::Bool(false)), token.span))
        }
        TokenKind::LeftParen => {
            parser.advance();
...
fn parse_builtin_unreachable(parser: &mut Parser) -> Result<ParsedNodeRef, ParseError> {
    let start = parser.expect(TokenKind::BuiltinUnreachable)?.span.start();
    parser.expect(TokenKind::LeftParen)?;
    let end = parser.expect(TokenKind::RightParen)?.span.end();
