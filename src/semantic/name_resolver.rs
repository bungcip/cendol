    ...
    fn visit_children<'a>(&mut self, ast: &'a Ast, node: &Node, context: &mut NameResolutionContext<'a>) {
        match &node.kind {
            ...
            NodeKind::EnumConstant(_, value_expr) => {
                if let Some(expr_ref) = value_expr {
                    self.walk_ast(ast, *expr_ref, context);
                }
            }
...
impl<'ast> SemanticVisitor<'ast> for NameResolver {
    ...
    fn visit_ident(&mut self, name: Symbol, resolved_symbol: &Cell<Option<SymbolEntryRef>>, span: SourceSpan, context: &mut Self::Context) {
        debug!(
        ...
        if let Some((symbol_ref, scope_id)) = context.symbol_table.lookup_symbol_from(name, context.current_scope) {
            ...
            context.diag.report_error(SemanticError::UndeclaredIdentifier {
                name,
                location: span,
            });
        }
