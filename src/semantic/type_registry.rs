        ...
        let layout = match type_kind {
            TypeKind::Builtin(b) => match b {
                ...
                BuiltinType::LongDouble => {
                    if self.target_triple.architecture == target_lexicon::Architecture::X86_64 {
                        // Temporarily downgrade long double to 8 bytes (double) because
                        // Cranelift backend doesn't fully support f128/f80 yet.
                        TypeLayout {
                            size: 8,
                            alignment: 8,
                            kind: LayoutKind::Scalar,
                        ...
                        // For other architectures (like aarch64), usually 16 bytes
                        TypeLayout {
                            size: 16,
                            alignment: 16,
                            kind: LayoutKind::Scalar,
