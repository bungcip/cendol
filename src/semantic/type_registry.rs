...
impl TypeRegistry {
    ...
    pub(crate) fn find_composite_type(&self, a: QualType, b: QualType) -> Option<QualType> {
        ...
        match (&type_a.kind, &type_b.kind) {
            (
                ...
                let (composite_params, composite_var, composite_proto) = if !proto_a {
                    (
                        params_b.iter().map(|p| p.param_type).collect::<Vec<_>>(),
                        *var_b,
                        *proto_b,
                    )
                } else if !proto_b {
                    (
                        params_a.iter().map(|p| p.param_type).collect::<Vec<_>>(),
                        *var_a,
                        *proto_a,
                    )
                } else {
                ...
                let key = FnSigKey {
                    ...
                    is_variadic: composite_var,
                    has_prototype: composite_proto,
                    is_noreturn: *nor_a,
                };
    ...
    pub(crate) fn builtin_function_type(&mut self, name: BuiltinFunctionKind) -> TypeRef {
        ...
        self.function_type(ret_ty, params, is_variadic, true, is_noreturn)
    }
    ...
    pub(crate) fn is_compatible(&self, mut a: QualType, mut b: QualType) -> bool {
        ...
        let class_a = ty_a.class();
        let class_b = ty_b.class();
        ...
        if class_a != class_b {
            ...
            // GCC and many other compilers are permissive with enum compatibility.
            if class_a == TypeClass::Enum && class_b == TypeClass::Builtin {
                return b.is_integer() && self.get_layout(ty_a).size == self.get_layout(ty_b).size;
            }
            if class_b == TypeClass::Enum && class_a == TypeClass::Builtin {
                return a.is_integer() && self.get_layout(ty_a).size == self.get_layout(ty_b).size;
            }
            return false;
        }

        // Bolt ⚡: Optimized dispatch based on TypeClass to avoid redundant property checks and lookups.
        // Terminal types (Builtin, Record, Complex) with different TypeRefs are never compatible.
        match class_a {
            TypeClass::Pointer => {
                let pa = self.get_pointee(ty_a).expect("is_pointer but no pointee");
                let pb = self.get_pointee(ty_b).expect("is_pointer but no pointee");
                self.is_compatible(pa, pb)
            }
            TypeClass::Array => {
                let ea = self.get_array_element(ty_a).expect("is_array but no element");
                let eb = self.get_array_element(ty_b).expect("is_array but no element");
                ...
                match (sa, sb) {
                    (ArraySizeType::Incomplete, _)
                    | (_, ArraySizeType::Incomplete)
                    | (ArraySizeType::Star, _)
                    | (_, ArraySizeType::Star)
                    | (ArraySizeType::Variable(_), _)
                    | (_, ArraySizeType::Variable(_)) => true,
                    (ArraySizeType::Constant(sa_val), ArraySizeType::Constant(sb_val)) => sa_val == sb_val,
                }
            }
            TypeClass::Function => {
                let type_a = &self.types[ty_a.index()];
                let type_b = &self.types[ty_b.index()];
    ...
    pub(super) fn composite_type(&mut self, mut a: QualType, mut b: QualType) -> Option<QualType> {
        if a == b {
            return Some(a);
        }
    ...
    fn composite_function_type(&mut self, a: QualType, type_a: &Type, type_b: &Type) -> Option<QualType> {
        let (ret_a, params_a, var_a, proto_a, noreturn_a) = match &type_a.kind {
            TypeKind::Function {
                return_type,
                parameters,
                is_variadic,
                has_prototype,
                is_noreturn,
            } => (*return_type, parameters, *is_variadic, *has_prototype, *is_noreturn),
            _ => unreachable!(),
        };
        let (ret_b, params_b, var_b, proto_b, noreturn_b) = match &type_b.kind {
            TypeKind::Function {
                return_type,
                parameters,
                is_variadic,
        ...
        let (composite_params, composite_var, composite_proto) = if !proto_a {
            (params_b.to_vec(), var_b, proto_b)
        } else if !proto_b {
            (params_a.to_vec(), var_a, proto_a)
        } else {
            if var_a != var_b {
                return None;
            }
