...
fn test_parser_errors() {
    let cases = [
        ...
        ("int foo[5]();", "Declaration not allowed in this context"),
        ("int (***+x);", "expected ), found Plus"),
        ("void foo(int +);", "expected ), found Plus"),
        ("void foo(int (int +));", "expected ), found Int"),
        ("void foo(int (int x +));", "expected ), found Int"),
        ("void foo(int (+));", "expected ), found Plus"),
        ("void foo(+);", "expected ), found Plus"),
        ("void foo(+x);", "expected ), found Plus"),
        ("void foo(int x, +);", "expected ), found Plus"),
        ("int foo(){foo(1}", "expected )"),
        ("void foo() { int x[] = {1, 2; }", "expected }"),
        ("void foo() { struct S { int x; ", "expected }"),
        ("int foo(int x, int y", "expected )"),
        ("void foo(int ( + ));", "expected ), found Plus"),
        ("int foo(int ..., int);", "expected ), found Ellipsis"),
        // B. Context-specific "expected identifier" errors
        (
            "void foo() { struct S s; s . 1; }",
            "expected identifier, found IntegerConstant",
        ),
        (
            "void foo() { struct S *s; s->1; }",
            "expected identifier, found IntegerConstant",
        ...
        ("enum E { 1 };", "expected identifier, found IntegerConstant"),
        (
            "struct S { int x; }; void foo() { struct S s = { . + = 1 }; }",
            "expected identifier, found Plus",
        ),
        ("void foo() { goto 1; }", "expected identifier, found IntegerConstant"),
