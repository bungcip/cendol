...
use crate::semantic::const_eval::{eval_const_expr, ConstEvalCtx};
...
#[test]
fn test_arithmetic() {
    snapshot_const_eval_batch("arithmetic", &[
        "1 + 2",
        "10 - 5",
        "2 * 3",
        "10 / 2",
...
#[test]
fn test_bitwise() {
    snapshot_const_eval_batch("bitwise", &[
        "1 << 2",
        "8 >> 1",
        "0x0F & 0xF0",
        "0x0F | 0xF0",
...
#[test]
fn test_logical() {
    snapshot_const_eval_batch("logical", &[
        "1 && 1",
        "1 && 0",
        "1 || 0",
        "0 || 0",
...
#[test]
fn test_comparisons() {
    snapshot_const_eval_batch("comparisons", &[
        "1 < 2",
        "2 > 1",
        "1 <= 1",
        "2 >= 2",
...
#[test]
fn test_ternary() {
    snapshot_const_eval_batch("ternary", &[
        "1 ? 10 : 20",
        "0 ? 10 : 20",
    ]);
}
...
#[test]
fn test_sizeof() {
    snapshot_const_eval_batch("sizeof", &[
        "sizeof(int)",
        "sizeof(long)",
        "sizeof(long long)",
...
#[test]
fn test_complex() {
    snapshot_const_eval_batch("complex", &[
        "1 + 2 * 3",
        "(1 + 2) * 3",
    ]);
}
...
fn test_overflow_wrapping() {
    ...
    snapshot_const_eval_batch("overflow_wrapping", &[
...
fn test_generic_selection() {
    snapshot_const_eval_batch("generic_selection", &[
