...
fn run_c11_test(test_content: &str) -> Result<String, String> {
    // Use a deterministic temp file path
    let test_file = "tests/temp_test_deterministic.c";
    
    // Write test file
    fs::write(test_file, test_content)
        .map_err(|e| format!("Failed to write test file: {}", e))?;
    
    // Run cargo with better error handling
    let output = Command::new("cargo")
        .args(&["run", "--quiet", "--", "--dump-parser", test_file])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
...
#[test]
fn test_basic_enum() {
    let test_content = r#"enum Color { RED, GREEN, BLUE };
int main() {
    enum Color c = RED;
    return 0;
}"#;

    let result = run_c11_test(test_content);
    match result {
        Ok(output) => {
            if !output.trim().is_empty() {
                assert_snapshot!(output);
            } else {
                // Empty output means compilation failed, snapshot should be empty
                assert_snapshot!("");
            }
        }
        Err(e) => {
            // Compilation error - snapshot the error
            assert_snapshot!(format!("ERROR: {}", e));
        }
    }
}
...
fn test_c11_restrict() {
    let test_content = r#"void copy_string(char* restrict dest, const char* restrict src) {
    int i;
    for (i = 0; src[i] != '\0'; i++) {
        dest[i] = src[i];
    }
...
fn test_thread_local() {
    let test_content = r#"_Thread_local int global_counter = 0;
...
fn test_generic_selection() {
    let test_content = r#"#define max(x, y) _Generic((x), \
    int: ((x) > (y) ? (x) : (y)), \
    default: 0 \
)
...
#[test]
fn test_noreturn() {
    let test_content = r#"_Noreturn void fatal_error() {
    while (1) {
...
fn test_comprehensive_c11() {
    let test_content = r#"// Test multiple C11 features in one file
